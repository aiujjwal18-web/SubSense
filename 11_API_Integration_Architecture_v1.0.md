...
| Depends On | 09_Implementation_Readiness_v1.0, 10_Database_Architecture_v1.0, 12_Backend_Architecture_v1.0 |
...
| --- | --- | --- |
| A — Direct Data Access | Supabase client (`supabase-js`) from Lovable frontend, governed entirely by RLS policies from `10_Database_Architecture_v1.0` | All user-owned CRUD: profile, preferences, subscriptions, shared subscriptions, members, payment requests, reading reminders/notifications/AI output/catalog |
...
Authentication: Supabase Auth, Google Sign-In primary, email/password fallback, per `01_Product_Strategy_v1.2`.
...
Authorization for Path A is enforced entirely by RLS policies already specified per-table in `10_Database_Architecture_v1.0`. Authorization for Path B is enforced by each Edge Function validating the caller's JWT `sub` (user id) against the resource being acted on before using its service-role privileges.
...
| `payment_requests` | select, insert, update status (pending/paid) | Owner or linked member | Payment status tracking |
...
**Reminder generation**: a trigger on `subscriptions` insert/update calls `generate_default_reminders()`, which reads the user's `reminder_default_days` from `user_preferences` and creates the corresponding rows in `reminders` (7-day, 2-day, renewal-day, per `01_Product_Strategy_v1.2` Reminder Strategy). This is the "Reminder Service" from doc 12, implemented as a database function rather than an application-layer class — same responsibility, same rule ownership, no separate service code to maintain.
...
- **Behavior:** Verifies the signature server-side against the Razorpay secret (never sent to the client). On success, writes `payment_transactions` and updates the user's premium status. On failure, writes a failed transaction record and returns a user-safe error.
...
Requirement → Table/Function → Enforcement Mechanism → Test Case (per `14_Testing_Strategy_v1.0`):
...
| v1.0 | Current | Real, buildable API contract under Lean Access Architecture (DEC-031), fully traced to existing frozen documents 00-10 and 12-16. |
